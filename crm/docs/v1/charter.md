...
| D31 | Follow-ups management (inline on dashboard) + overdue alert chip | Tomás | ✅ Done 2026-04-25 — migration `0034` adds `follow_up_status` enum (`pending` \| `done` \| `dismissed`, default `pending`). Tabs + list rendered **inline** on `/dashboard` (below Sala de espera, hidden for groomer; `?followUpsTab=pending\|done\|dismissed`). Per-row actions "Marcar atendido" / "Descartar" / "Reabrir". Third AlertChip on dashboard (`Seguimientos vencidos`, admin/owner only) anchor-scrolls to the section. Cron job updated to skip non-pending rows so dismissed follow-ups stop auto-mailing. Smoke seed extended with 6 follow-up rows (3 overdue pending, 1 future, 1 done, 1 dismissed). Initially shipped as a dedicated `/dashboard/follow-ups` route, then folded inline per Tomás's preference for fewer top-level pages. |
