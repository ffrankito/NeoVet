...
- [ ] Flujo de estética completo probado: crear turno de estética → asignar esteticista → esteticista registra sesión → fotos subidas → hallazgos registrados → ingreso en caja automático
- [ ] Cada rol probado en aislamiento (ingresar como veterinario, como esteticista — verificar que dashboard filtra turnos y las rutas bloqueadas redirigen)
...
- [ ] Caja: abrir sesión → registrar movimientos → ventas + estética se reflejan en balance → cerrar con conteo de efectivo
...
- [ ] Cargos y deudores: auto-cargo al registrar consulta/estética/venta → página deudores muestra saldo → registrar pago parcial/total → estado se actualiza
...
- **Estética** — cada perro que pasa por estética tiene su perfil propio (comportamiento, tipo de pelaje, tiempo estimado). El esteticista registra cada sesión con fotos antes/después, hallazgos, precio final y método de pago. El ingreso se registra automáticamente en la caja abierta.
...
- **Cargos y deudores** — cargos por consulta, estética, procedimiento, venta, internación u otro. Pagos parciales y totales. Página "Deudores" con clientes con saldo pendiente. Auto-cargo al registrar consultas, sesiones de estética y ventas.
...
| Rol | Qué puede hacer |
|---|---|
| **Admin / Owner** (Paula, recepción) | Todo — clientes, pacientes, turnos, historia clínica, internaciones, procedimientos, consentimientos, estética, pet shop, caja, cargos y deudores, staff, configuración. Dashboard muestra todos los turnos del día + estado de caja |
...
| **Esteticista** | Ver turnos de estética asignados · Registrar sesiones de estética (ingreso auto en caja) · Editar perfil de estética del paciente |
...
| Registrar sesión de estética | Manual | Esteticista |
...
| Ingreso de estética → movimiento de caja | Automático | Sistema (si hay caja abierta) |
...
| Auto-cargo al registrar consulta, estética o venta | Automático | Sistema |
...
| El esteticista no ve sus turnos | El turno no está tipificado como "grooming" o no está asignado al esteticista | Un admin edita el turno y cambia el tipo o la asignación |
| El ingreso de estética no aparece en caja | No había caja abierta al registrar la sesión | Abrir caja antes de registrar sesiones. Si ya se registró, agregar el movimiento manualmente |
...
| Error al emitir factura en ARCA | *(Fase D diferida a post-lanzamiento — esta fila aplica cuando se implemente facturación)* | — |
| No puedo subir una foto en la sesión de estética | Archivo muy grande o formato no soportado | Usar JPG o PNG, máximo 10 MB |
...
| Los hallazgos del esteticista no alertan al veterinario | Flujo pendiente de entrevista con el esteticista (v2) | El esteticista le avisa al veterinario manualmente |
...
- Recordatorios de turno, estética y vacunas por WhatsApp
- Confirmación de turno por WhatsApp
- Alertas al veterinario por hallazgos del esteticista

### v3 — Inteligencia y automatización avanzada
- Dictado por voz para historia clínica
- Tiempo estimado de estética calculado automáticamente
