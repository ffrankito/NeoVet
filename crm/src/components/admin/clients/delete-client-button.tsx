...
const ROW_LABELS: Array<{ key: keyof ClientDeletePreview; label: string }> = [
  ...
  { key: "pendingCharges", label: "Cargos pendientes" },
];

export function DeleteClientButton({ clientId }: DeleteClientButtonProps) {
  const [open, setOpen] = useState(false);
  const [isPending, setIsPending] = useState(false);
  const [preview, setPreview] = useState<ClientDeletePreview | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getClientDeletePreview(clientId)
      .then((p) => {
          ...
          {previewError ? (
            ...
            visibleRows.length > 0 ? (
              ...
              <p className="text-muted-foreground">
                Este cliente no tiene registros asociados.
              </p>
            )
          ) : (
