...
4. **L4 keywords are in Argentine Spanish.** Canonical list lives in [chatbot/src/lib/whatsapp/agent.ts](chatbot/src/lib/whatsapp/agent.ts) (`L4_KEYWORDS` array, ~145 entries across 12 categories) and is **mirrored** as a categorized prose summary in [chatbot/src/lib/prompts/whatsapp-system.ts](chatbot/src/lib/prompts/whatsapp-system.ts) (the "Urgencias" section). The web widget's [chatbot/src/lib/prompts/system.ts](chatbot/src/lib/prompts/system.ts) does **not** carry the keyword list — the web widget has no L4 fast-path; it just routes emergencies to WhatsApp. Both keyword-bearing files must stay in sync by hand. Categories (with examples): respiratorio (`no respira`, `le cuesta respirar`, `boquea`), cianosis (`mucosas azules`, `lengua azul`), hemorragia (`hemorragia activa`, `pierde sangre por la vulva`), obstrucción urinaria (`gato obstruido`, `no puede hacer pis`), trauma (`atropellado`, `fractura`, `no apoya la pata`), intoxicación (`espuma por la boca`, `me lo envenenaron`), neurológico (`convulsión`, `desmayado`, `no reacciona`), obstétricas (`distocia`, `cachorro trabado`, `líquido verde`), golpe de calor (`no para de jadear`, `se ahoga del calor`), vómito/diarrea graves (`vomita sangre`, `diarrea con sangre`), dolor severo / colapso (`no se levanta`, `parece muerto`), señales generales (`emergencia`, `se está muriendo`, `sin pulso`). Full Paula-confirmed list shipped 2026-04-27. When updating, update both files in the same commit.
